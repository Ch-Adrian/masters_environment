        ...
        for algo, modes in algos.items():
            for mode in modes:
                for p in parallelism:
                    cmds.append((algo, str(mode), data_file, str(data_len[data_file][mode]), str(p)))
...
def run_algorithm(num, data_loading_change=True, algo_implementation_change=True):
    ...
    flink_cmd = ["bash", "run_job.sh", cmd[0], cmd[1], cmd[3], cmd[4]]
    stop_flink_job = ["bash", "stop_job.sh"]
    foreground_scanning_topic = ["java", "-jar", "../jars/data-scanner.jar"]

    # Compile and copy algorithm jar if needed
    if algo_implementation_change:
        copy_jar_cmd = ["bash", "copy_jars.sh"]
        copy_jar_process = subprocess.run(copy_jar_cmd, check=True)
    ...
    try:
        ...
        amt_of_entries = 0
        timeStart = time.time()
        amt_of_entries_begin = -1
        while amt_of_entries != amt_of_entries_begin:
            time.sleep(10)
            amt_of_entries_begin = amt_of_entries
        ...
        plot_metrics(cmd[0].split("-1.3.0")[0], cmd[4], cmd[2].split(".")[0])
...
if __name__ == "__main__":
    ...
    run_algorithm(6, False, False)
    # cmd = generate_commands(8)
    # plot_metrics(cmd[0].split("-1.3.0")[0], cmd[4], cmd[2].split(".")[0])
    # plot_all()